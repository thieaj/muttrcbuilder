package Mutt::Config::Version::v1_5_18;
...
If unset, <a href="http://www.mutt.org/doc/manual/manual-6.html#charset">$charset</a> value will be used instead.
...
<dt><strong>%*X </strong></dt>
<dd><p>soft-fill with character "X" as pad
</p></dd>
</dl>
...
<p>
For an explanation of `soft-fill\', see the ``<a href="http://www.mutt.org/doc/manual/manual-6.html#index-format">$index_format</a>\'\' documentation.
</p>
'
},
...
``<a href="http://www.mutt.org/doc/manual/manual-6.html#lists">lists</a>\'\' commands).  When set, if the ``Reply-To:\'\' field is
...
``<a href="http://www.mutt.org/doc/manual/manual-6.html#index-format">$index_format</a>\'\' for supported printf()-style sequences.
</p>
'
},
...
<dt><strong>%f </strong></dt>
<dd><p>sender (address + real name), either From: or Return-Path:
</p></dd>
<dt><strong>%F </strong></dt>
...
<dd><p>progress indicator for the builtin pager (how much of the file has been displayed)
</p></dd>
<dt><strong>%s </strong></dt>
...
``<a href="http://www.mutt.org/doc/manual/manual-6.html#save-hook">save-hook</a>\'\', ``<a href="http://www.mutt.org/doc/manual/manual-6.html#fcc-hook">fcc-hook</a>\'\' and ``<a href="http://www.mutt.org/doc/manual/manual-6.html#fcc-save-hook">fcc-save-hook</a>\'\', too.
</p>
<p>
...
``<a href="http://www.mutt.org/doc/manual/manual-6.html#recall">$recall</a>\'\' variable.
</p>
'
},
...
<dt><strong>%c  </strong></dt>
<dd><p>current entry number
</p></dd>
<dt><strong>%e  </strong></dt>
<dd><p>extra information *
</p></dd>
<dt><strong>%n  </strong></dt>
<dd><p>destination name
</p></dd>
<dt><strong>%t  </strong></dt>
<dd><p>``*\'\' if current entry is tagged, a space otherwise
</p></dd>
<dt><strong>%>X </strong></dt>
...
<em>read_inc</em> messages have been read or searched (e.g., if set to 25, Mutt will
...
<p>
Also see the ``<a href="http://www.mutt.org/doc/manual/manual-6.html#write-inc">$write_inc</a>\'\' variable and the ``Tuning\'\' section of the
...
If ``<a href="http://www.mutt.org/doc/manual/manual-6.html#envelope-from-address">$envelope_from_address</a>\'\' is set, it will be used as the sender
...
#
#   This variable is a colon-separated list of character encoding schemes
#   for text file attachments. If unset, $charset value will be used
...
#   see if it has already been looked at. If check_new is unset, no check
...
#   from mailing lists (as defined by the ``subscribe'' or ``lists''
...
#   new mail. See also the ``mailboxes'' command.
#
#  imap_delim_chars
#
#   Type: string
...
#
#   %X
#          number of attachments (please see the ``attachments'' section
...
#   Default: yes
#
#   Controls whether or not mutt marks newunread messages as old if you
...
#   than pager_index_lines, then the index will only use as many lines as
#   it needs.
#
#  pager_stop
#
...
#   by use of the smime-menu. (Crypto only)
#
#  crypt_autoencrypt
#
#   Type: boolean
...
#   in ``$crypt_replysign'', that mutt is not able to find out whether an
...
#   is only used if ``$mime_forward'' is set and ``$mime_forward_decode''
#   is unset. (PGP only)
#
#  smime_timeout
#
...
#   you for your password when you open POP mailbox. Warning: you should
...
#   need to set this to a regular expression that matches exactly the quote
...
#   such as search and limit. The message is printed after read_inc
...
#   ``$folder'' directory with the username part of the recipient address).
#   If the mailbox exists, the outgoing message will be saved to that
#   mailbox, otherwise the message is saved to the ``$record'' mailbox.
#
...
#   ``smtp_url'' to configure mutt to send mail via SMTP. Warning: you
...
#   set sort=reverse-threads.) Note: For reversed ``$sort'' order $sort_aux
...
#   is nonzero. optional_stringmay contain other sequences as well as
...
#   ``patterns'' above. If set, the headers and attachments of messages to
...
#   controls the frequency with which progress updates are displayed. It
#   suppresses updates less than ``time_inc'' milliseconds apart. This can
#   improve throughput on systems with slow terminals, or when running mutt
#   on a remote system.
#
#  timeout
#
...
#   address is specified in the CC header field, but you are not the only
#   recipient. The fifth character is used to indicate mail that was sent
#   by you. The sixth character is used to indicate when a mail was sent to
...
#   ``$sendmail'' variable already contains "-f" or if the executable
...
#   deliver directly via SMTP (see ``$smtp_url''), this option does
